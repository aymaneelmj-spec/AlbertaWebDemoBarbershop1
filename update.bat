...
>>fix.ps1 echo   ^</svg^>
>>fix.ps1 echo );
>>fix.ps1 echo const explodeItems = [
>>fix.ps1 echo   { x: -300, y: -200 }, { x: 300, y: -200 },
>>fix.ps1 echo   { x: -300, y: 200 }, { x: 300, y: 200 },
>>fix.ps1 echo   { x: 0, y: -400 }, { x: 0, y: 400 }
>>fix.ps1 echo ];
...
>>fix.ps1 echo   return (
>>fix.ps1 echo     ^<div^>
>>fix.ps1 echo       ^<audio ref={audioRef} src="/audiobarber.wav" loop /^>
...
>>fix.ps1 echo             {explodeItems.map((e, i) =^> (
>>fix.ps1 echo               ^<motion.div key={i} style={{ position: 'absolute' }} initial={{ x: 0, y: 0, opacity: 0, scale: 0 }} animate={{ x: e.x, y: e.y, opacity: [0, 1, 0], scale: [0, 1.5, 0], rotate: 360 }} transition={{ duration: 4, repeat: Infinity, delay: i * 0.1 }}^>
